...
export function ProjectImage({ src, title, category, priority = false }: ProjectImageProps) {
  ...
  return (
    <div className="relative aspect-[16/10] overflow-hidden rounded-lg border border-ink/10 bg-fog shadow-sm">
...
function Placeholder({ title, category }: { title: string; category?: string }) {
  return (
    <div className="relative aspect-[16/10] overflow-hidden rounded-lg border border-ink/10 bg-[#171712] shadow-sm">
...
function PlaceholderContent({ title, category }: { title: string; category?: string }) {
  return (
    <>
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_20%_20%,rgba(189,111,77,0.34),transparent_30%),radial-gradient(circle_at_86%_18%,rgba(15,118,110,0.26),transparent_28%),linear-gradient(135deg,#171712_0%,#2d2d2d_48%,#ece7dc_180%)]" />
      <div className="absolute inset-0 opacity-[0.08] [background-image:linear-gradient(rgba(255,255,255,0.75)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,0.75)_1px,transparent_1px)] [background-size:28px_28px]" />
      <div className="absolute left-5 right-5 top-5 flex items-center justify-between gap-3">
        <div className="flex items-center gap-2 rounded-full border border-white/12 bg-white/10 px-3 py-1.5 text-xs font-bold text-white/80 backdrop-blur">
          <Layers3 className="h-3.5 w-3.5 text-clay" />
          {category ?? "Projet web"}
        </div>
      ...
      <div className="absolute bottom-5 left-5 right-5 grid gap-4 sm:grid-cols-[1fr_150px] sm:items-end">
        <div>
          <div className="mb-4 flex gap-2">
            <span className="h-2 w-12 rounded-full bg-clay" />
            <span className="h-2 w-7 rounded-full bg-teal" />
            <span className="h-2 w-16 rounded-full bg-white/25" />
          </div>
          <p className="max-w-[18rem] text-2xl font-black leading-tight text-white">{title}</p>
          <p className="mt-2 text-xs font-semibold uppercase text-white/50">Capture à intégrer</p>
        </div>
        <div className="hidden rounded-md border border-white/12 bg-white/10 p-4 text-white shadow-sm backdrop-blur sm:block">
          <MonitorSmartphone className="mb-5 h-7 w-7 text-clay" />
